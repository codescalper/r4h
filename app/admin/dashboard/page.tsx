...
import GalleryMultiUpload, {
  type GalleryTag as GalleryTagType,
} from "@/components/admin/gallery-multi-upload";
import { SearchableRecipientSelect } from "@/components/admin/searchable-recipient-select";
  ...
  async function handleMakeAdmin(m: Member) {
    ...
    try {
      ...
      if (res.ok) {
        setMakeAdminMsg({
          id: m.id,
          msg: `Admin account created for ${m.email}. They can now log in at /admin/login.`,
          ok: true,
        });
      } else {
        setMakeAdminMsg({
          id: m.id,
          msg: data.error || "Failed to create admin.",
          ok: false,
        });
      }
    } finally {
                ...
                <Fragment key={m.id}>
                  ...
                  {makeAdminFor === m.id && (
                    <tr>
                      <td
                        colSpan={6}
                        className="px-4 py-3 bg-primary/5 dark:bg-primary/10 border-t border-primary/20"
                      >
                        <div className="flex items-center gap-3 flex-wrap">
                          ...
                          <button
                            ...
                            disabled={
                              makeAdminLoading || makeAdminPwd.length < 8
                            }
                  ...
                  {makeAdminMsg?.id === m.id && (
                    <tr>
                      <td
                        colSpan={6}
                        className={`px-4 py-2 text-sm font-medium ${makeAdminMsg.ok ? "bg-primary/5 text-primary" : "bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400"}`}
                      >
                        {makeAdminMsg.msg}
                        <button
                          onClick={() => setMakeAdminMsg(null)}
...
function AdminsTab() {
  ...
  async function handleCreate(e: FormEvent) {
    ...
    try {
      ...
      if (res.ok) {
        setMsg({
          text: `Admin "${form.name}" (${form.email}) created successfully.`,
          ok: true,
        });
  ...
  async function handleDelete(id: string) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setMsg({ text: data.error || "Failed to remove admin.", ok: false });
      }
    } finally {
    ...
    <div className="space-y-8">
      ...
      <div className="bg-card rounded-2xl border border-border p-6 shadow-sm">
        ...
        <p className="text-sm text-muted-foreground mb-4">
          Create a new admin account or promote an approved member by entering
          their email.
        </p>
        {msg && (
          <div
            className={`mb-4 px-4 py-2.5 rounded-lg text-sm font-medium flex items-start justify-between gap-3 ${msg.ok ? "bg-primary/10 text-primary" : "bg-destructive/10 text-destructive"}`}
          >
            <span>{msg.text}</span>
            <button
              onClick={() => setMsg(null)}
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-border">
                {admins.map((a) => (
                  <tr
                    key={a.id}
                    className="bg-card hover:bg-muted/30 transition"
                  >
                    <td className="px-4 py-3">
...
function MessagesTab() {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      const [m, mems] = await Promise.all([
        ...
        fetch("/api/admin/members?status=APPROVED&limit=200").then((r) =>
          r.json(),
        ),
      ]);
  ...
  const filtered = messages.filter(
    (m) => scopeFilter === "ALL" || m.scope === scopeFilter,
  );
          ...
          {preview && (
            <div className="space-y-3">
              <div className="flex items-center gap-2 text-xs text-muted-foreground flex-wrap">
                ...
                <span>
                  To: {preview.member.firstName} {preview.member.lastName} (
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        ...
        />
        {/* Donations stat hidden: <StatCard label="Total Donations" value={`₹${stats.totalDonationsAmount.toLocaleString()}`} icon={Heart} accent="border-primary/20" /> */}
      </div>
      <div className="relative overflow-hidden rounded-2xl border border-primary/20 bg-primary/5 p-6">
        <div
          ...
          style={{
            backgroundImage:
              "radial-gradient(circle at 90% 50%, var(--color-primary) 0%, transparent 60%)",
          }}
        />
          ...
          {stats.pendingMembers > 0 && (
            <div className="mt-3 inline-flex items-center gap-2 bg-yellow-100 dark:bg-yellow-900/30 border border-yellow-200 dark:border-yellow-800 px-3 py-1.5 rounded-full">
              ...
              <span className="text-xs font-semibold text-yellow-700 dark:text-yellow-400">
                {stats.pendingMembers} awaiting action
              </span>
            </div>
          )}
        </div>
  ...
  const fetchSubmissions = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/admin/contact${unreadOnly ? "?unread=true" : ""}`,
      );
      const data = await res.json();
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <Button variant="outline" size="sm" onClick={fetchSubmissions}>
            <Loader2
              className={`w-3.5 h-3.5 mr-1.5 ${loading ? "animate-spin" : "hidden"}`}
            />
            Refresh
          </Button>
        </div>
        ...
        <div className="space-y-2">
          {submissions.map((s) => (
            ...
            >
              ...
              >
                <div
                  className={`w-2 h-2 rounded-full shrink-0 ${s.read ? "bg-muted-foreground/30" : "bg-primary"}`}
                />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span
                      className={`text-sm font-medium text-foreground ${!s.read ? "font-semibold" : ""}`}
                    >
                      {s.name}
                    </span>
                    <span className="text-xs text-muted-foreground">
              ...
              {expandedId === s.id && (
                <div className="px-4 pb-4 border-t border-border/50">
                  <p className="text-xs text-muted-foreground mt-3 mb-1 uppercase tracking-wide">
                    Message
                  </p>
...
function PostsTab() {
  ...
  const [writeCategory, setWriteCategory] =
    useState<PostCategory>("ANNOUNCEMENT");
  const [writeContent, setWriteContent] = useState("");
  const [writeCover, setWriteCover] = useState<File | null>(null);
  const [writeCoverPreview, setWriteCoverPreview] = useState("");
  const [writeSubmitting, setWriteSubmitting] = useState(false);
  const [writeError, setWriteError] = useState("");
  const [writeSuccess, setWriteSuccess] = useState(false);
  const writeCoverRef = useRef<HTMLInputElement>(null);
  ...
  const handleWriteSubmit = async () => {
    ...
    if (!writeContent.trim() || writeContent === "<p></p>") {
      setWriteError("Post content cannot be empty.");
      return;
    }
    setWriteError("");
      ...
      <div className="rounded-xl border border-border bg-white dark:bg-zinc-900 overflow-hidden">
        ...
        >
          <span className="flex items-center gap-2">
            <Pencil className="w-4 h-4 text-primary" /> Write Official Post
          </span>
          <span className="text-xs text-muted-foreground">
            {showWrite ? "Collapse ▲" : "Expand ▼"}
          </span>
        </button>
        {showWrite && (
          <div className="px-5 pb-5 border-t border-border space-y-4 pt-4">
            <div className="flex items-start gap-2 bg-primary/5 border border-primary/20 rounded-lg px-3 py-2">
              ...
              <p className="text-xs text-muted-foreground">
                Posts published here are <strong>immediately approved</strong>{" "}
                and visible to all members as official announcements.
              </p>
            </div>
            {writeSuccess && (
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2">
                ...
                <Input
                  className="mt-1"
                  placeholder="e.g. Run4Health Season 3 is here!"
                  value={writeTitle}
              ...
              <div>
                ...
                <Select
                  value={writeCategory}
                  onValueChange={(v) => setWriteCategory(v as PostCategory)}
                >
                  ...
                  <SelectContent>
                    {(
                      Object.entries(POST_CAT_LABELS) as [
                        PostCategory,
                        string,
                      ][]
                    ).map(([v, l]) => (
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setWriteCover(f);
                    setWriteCoverPreview(f ? URL.createObjectURL(f) : "");
                  }}
                />
                <label
                  htmlFor="write-cover"
                  className={`mt-1 flex items-center gap-2 border-2 rounded-lg px-3 py-2.5 cursor-pointer transition-all ${writeCoverPreview ? "border-primary/40 bg-primary/5" : "border-dashed border-border hover:border-primary/40"}`}
                >
                  {writeCoverPreview ? (
                    <img
                      src={writeCoverPreview}
                      alt=""
                  ...
                  <span className="text-sm text-muted-foreground truncate">
                    {writeCover ? writeCover.name : "Upload cover photo"}
                  </span>
                </label>
              </div>
            </div>
            <div>
              ...
              <div className="mt-1 border border-border rounded-lg overflow-hidden min-h-[200px]">
                <TipTapEditor
                  content={writeContent}
          ...
          {posts.map((p) => (
            ...
            >
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full font-medium ${POST_STATUS_COLORS[p.status]}`}
                  >
                    {p.status}
                  </span>
                  <span className="text-xs text-muted-foreground">
                    {POST_CAT_LABELS[p.category]}
                  </span>
                  <span className="text-xs text-muted-foreground">
                    ·{" "}
                    {p.member
                      ? `${p.member.firstName} ${p.member.lastName}`
                      : (p.admin?.name ?? "Unknown")}
                  </span>
                </div>
                ...
                {p.status === "PENDING" && (
                  <>
                    ...
                    <button
                      ...
                      className="p-1.5 rounded-lg text-destructive hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"
                      title="Reject"
                    >
        ...
        <DialogContent>
          ...
          <div className="space-y-3">
            ...
            <div className="flex gap-2">
              <Button
                onClick={() =>
                  rejecting && updateStatus(rejecting, "REJECTED", rejectNote)
                }
                className="flex-1"
              >
                Reject
            ...
            >
              ...
              <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                <button
                  ...
                  className="w-8 h-8 rounded-full bg-destructive flex items-center justify-center text-white hover:bg-destructive/80 transition-colors"
                >
                  <Trash2 className="w-3.5 h-3.5" />
                </button>
              </div>
...
function ProgramsAdminTab() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editId) {
        await fetch(`/api/admin/programs/${editId}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          ...
          <CardContent className="p-5 space-y-4">
            <div className="grid sm:grid-cols-2 gap-3">
              ...
              <div>
                ...
                <Select
                  value={category}
                  onValueChange={(v) => setCategory(v as ProgCategory)}
                >
                  <SelectTrigger className="mt-1">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    {(Object.keys(PROG_CAT_LABELS) as ProgCategory[]).map(
                      (c) => (
              ...
              <div>
                ...
                <Select
                  value={status}
                  onValueChange={(v) => setStatus(v as ProgStatus)}
                >
                  <SelectTrigger className="mt-1">
                    <SelectValue />
                  </SelectTrigger>
...
export default function AdminDashboardPage() {
  ...
  const navItems: Array<{
    ...
    // Donations tab hidden: { tab: 'donations', label: 'Donations', icon: Heart },
      ...
      <div className="flex min-h-screen w-full bg-background">
        ...
        <SidebarInset className="flex flex-col flex-1 min-w-0">
          ...
          <main className="flex-1 p-6 bg-background">
            ...
            {/* Donations tab hidden: {activeTab === 'donations' && <DonationsTab />} */}
