...
function ResetForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setSuccess(true);
      setTimeout(() => router.push('/member/login'), 2500);
    } catch {
  ...
  if (!token) return (
    <div className="text-center py-8">
      ...
      <a href="/member/forgot-password" className="mt-4 inline-block text-primary hover:underline text-sm">Request new link</a>
    </div>
  );
  ...
  return success ? (
    ...
    <>
      ...
      <p className="text-zinc-500 dark:text-zinc-400 text-sm mb-6">Choose a strong password for your member account.</p>
        ...
        <div>
          ...
          <div className="relative">
            <input type={showConfirm ? 'text' : 'password'} required value={confirm} onChange={e => setConfirm(e.target.value)}
              ...
              className={`w-full px-4 py-3 pr-12 rounded-xl border bg-zinc-50 dark:bg-zinc-800 text-zinc-900 dark:text-white placeholder-zinc-400 focus:outline-none focus:ring-2 focus:ring-primary/50 focus:border-transparent transition text-sm ${
