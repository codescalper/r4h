    ...
    <div className="min-h-screen pt-16 flex bg-background">
      ...
      <aside className="hidden lg:flex w-64 flex-col shrink-0 border-r border-border bg-card py-8 px-4">
        <div className="flex flex-col items-center gap-2 pb-6 border-b border-border">
          ...
          <p className={`${lora.className} font-semibold text-foreground`}>
            Rahul Sharma
          </p>
          <Badge
            variant="outline"
        ...
        <nav className="mt-6 space-y-1">
          {[
            ...
            >
              {n.icon}
              <span className={lora.className}>{n.label}</span>
            </button>
          ))}
        </nav>
      </aside>
        ...
        <div className="border-b border-border px-6 py-3 flex items-center justify-between bg-card/50">
          ...
          <div className="flex items-center gap-2">
            <Button size="sm" variant="outline" className="gap-1 text-xs">
              <LogIn className="w-3.5 h-3.5" /> Logout
            </Button>
          </div>
        </div>
        ...
        <div className="p-6 max-w-4xl">
          <AnimatePresence mode="wait">
            {activeTab === "profile" && (
              ...
              >
                <h2
                  className={`${bebasNeue.className} text-3xl tracking-wide text-foreground mb-6`}
                >
                  PROFILE
                </h2>
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between mb-5">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p
                            className={`${lora.className} font-semibold text-foreground`}
                          >
                            Rahul Sharma
                          </p>
                          <p
                      ...
                      >
                        {editing ? (
                          ...
                          <>
                            <Activity className="w-3.5 h-3.5" /> Edit Profile
                          </>
                        )}
                      </Button>
                    </div>
                    {editing ? (
                      <div className="grid sm:grid-cols-2 gap-3">
                        {[
                          ...
                          [
                            "Emergency Contact",
                            "Priya Sharma - +91 99999 11111",
                          ],
                        ].map(([l, v]) => (
                          <div key={l}>
                            <Label>{l}</Label>
                            <Input className="mt-1" defaultValue={v} />
                          </div>
                        ))}
                        <Button
                          className="sm:col-span-2 gap-1"
            ...
            {activeTab === "health" && (
              ...
              >
                <div className="flex items-center justify-between mb-6">
                  <h2
                    className={`${bebasNeue.className} text-3xl tracking-wide text-foreground`}
                  >
                    HEALTH DATA
                  </h2>
                  <Dialog open={healthOpen} onOpenChange={setHealthOpen}>
                    <DialogTrigger asChild>
                      <Button size="sm" className="gap-1">
                        <TrendingUp className="w-3.5 h-3.5" /> Add Update
                      </Button>
                    </DialogTrigger>
                    <DialogContent>
                      <DialogHeader>
                        <DialogTitle
                          className={`${bebasNeue.className} tracking-wide`}
                        >
                          Add Health Update
                        </DialogTitle>
                      </DialogHeader>
                ...
                <p
                  className={`${lora.className} text-xs text-muted-foreground bg-muted/50 rounded-md p-3 mb-5`}
                >
                  Update your health data every 3 months for best results
                </p>
                  ...
                  <CardContent>
                    <div className="flex items-end gap-3 h-24">
                      {healthHistory.map((h, i) => (
                        ...
                        >
                          <span
                            className={`${lora.className} text-xs text-primary font-medium`}
                          >
                            {h.weight}
                          </span>
                          <div
                            ...
                            style={{
                              height: `${(h.weight / maxWeight) * 80}px`,
                            }}
                          >
                      ...
                      <thead className="bg-muted/50">
                        <tr>
                          {[
                            ...
                            <th
                              key={h}
                              className={`${lora.className} text-left text-xs text-muted-foreground px-4 py-2.5 font-medium`}
                            >
                              {h}
                            </th>
                          ))}
                      ...
                      <tbody>
                        {healthHistory.map((h, i) => (
                          <tr key={h.date} className="border-t border-border">
                            <td
                              className={`${lora.className} px-4 py-2.5 text-foreground`}
                            >
                              {h.date}
                            </td>
                            <td
              ...
              >
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {[
                    ...
                    {
                      name: "ECG_Report_2024.pdf",
                      date: "Sep 5, 2024",
                      type: "Cardiac",
                    },
                    {
                      name: "BMI_Check_2024.jpg",
                      date: "Jun 20, 2024",
                      type: "General",
                    },
                  ].map((r) => (
                    <Card key={r.name} className="border-border">
                      <CardContent className="p-4">
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          <p
                            className={`${lora.className} text-xs font-medium text-foreground truncate`}
                          >
                            {r.name}
                          </p>
                        </div>
                        <Badge
                          variant="outline"
                          className="text-xs mb-2 border-primary/20 text-primary"
                        >
                          {r.type}
                        </Badge>
                        <p
                          className={`${lora.className} text-xs text-muted-foreground mb-3`}
                        >
                          {r.date}
                        </p>
