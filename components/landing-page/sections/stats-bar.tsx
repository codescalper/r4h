...
function StatItem({
  ...
  return (
    <div ref={ref} className="text-center">
      <p
        className={`${bebasNeue.className} text-4xl sm:text-5xl tracking-wider`}
      >
        {count}
        {suffix}
      </p>
      <p className={`${lora.className} text-xs sm:text-sm opacity-80 mt-1`}>
        {label}
      </p>
    </div>
  );
...
export default function StatsBar() {
  return (
    <section className="bg-primary py-10 sm:py-12 text-primary-foreground">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 grid grid-cols-2 sm:grid-cols-3 gap-6 sm:gap-8">
        {STATS.map((s) => (
